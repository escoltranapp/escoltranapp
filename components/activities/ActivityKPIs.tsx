  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
      {stats.map((s) => (
        <div
          key={s.label}
          className="bg-surface border border-border rounded-2xl p-6 hover:bg-foreground/[0.02] transition-all group overflow-hidden relative shadow-lg"
        >
          <div
            className="absolute -top-12 -right-12 w-24 h-24 blur-[40px] opacity-10 transition-opacity group-hover:opacity-20"
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div
              className="px-2 py-0.5 rounded-full text-[9px] font-black font-mono border uppercase tracking-widest"
              style={{ color: s.color, backgroundColor: `${s.color}05`, borderColor: `${s.color}10` }}
            >
              {s.trend}
            </div>
          </div>
          ...
          <div>
            <div className="text-[9px] font-mono text-secondary uppercase tracking-[0.3em] font-black mb-1 italic">
              {s.label}
            </div>
