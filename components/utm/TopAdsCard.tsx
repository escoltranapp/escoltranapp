  ...
  return (
    <Card className="bg-[#0A0A0A] border-white/5 h-full">
      ...
      <CardContent className="space-y-4">
        {data.map((row, i) => (
          <div key={row.name} className={`group p-4 rounded-2xl border transition-all duration-300 ${i === 0 ? 'bg-emerald-500/5 border-emerald-500/20 shadow-lg shadow-emerald-500/5' : 'bg-white/[0.02] border-white/5 hover:border-white/10'}`}>
            <div className="flex items-start justify-between gap-4 mb-3">
              ...
              <div className="text-right shrink-0">
                <div className={`text-sm font-black tracking-tight ${i === 0 ? 'text-emerald-400' : 'text-white/80'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className={`text-[9px] font-black uppercase h-5 ${i === 0 ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : 'bg-white/10 border-white/20 text-white/50'}`}>
                {row.won} ganhos
              </Badge>
            </div>
          </div>
