  ...
  return (
    <Card className="bg-[#0A0A0A] border-white/5 h-full">
      ...
      <CardContent className="space-y-4">
        {data.map((row, i) => (
          <div key={row.campaign} className={`group p-4 rounded-2xl border transition-all duration-300 ${i === 0 ? 'bg-blue-500/5 border-blue-500/20 shadow-lg shadow-blue-500/5' : 'bg-white/[0.02] border-white/5 hover:border-white/10'}`}>
            <div className="flex items-start justify-between gap-4 mb-3">
              <div className="flex flex-col gap-1 overflow-hidden">
                <span className="text-xs font-black text-white group-hover:text-blue-400 transition-colors truncate">
                   {row.campaign}
                </span>
              ...
              <div className="text-right shrink-0">
                <div className={`text-sm font-black tracking-tight ${i === 0 ? 'text-blue-400' : 'text-[#F97316]'}`}>
